...
description: >-
  ...
  chain, looping each checkpoint until the definition-of-done holds. Fires on any
  end-to-end build intent: an issue to take all the way, or a freeform goal stated
  with the whole chain. Unlike typing the chain by hand, it adversarially reviews
  the PLAN before code is written, gates plan-approval through the operator,
  exercises the REAL app, and never silently de-scopes.
