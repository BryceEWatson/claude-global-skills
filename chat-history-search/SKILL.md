...
description: "Exhaustive search across ALL of Bryce's local Claude chat history, both Cowork and the Claude Code CLI, to find past prompts, recover a conversation, inventory how often a pattern was used, or audit prior work across projects. Knows every log location, every JSONL line shape, and the false-positive gotchas that trip up naive grep."
---
...
- Pattern mining across sessions (use `pattern-retrospective` for that; the old `transcript-analysis` skill was retired 2026-09-06)
