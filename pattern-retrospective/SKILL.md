...
The chat-arch v2 Rev 3 plan describes the substrate that will eventually automate this. Until that ships (the chat-arch repo's `_planning/chat-arch-v2-rev3-plan.md`), you are the curator, falsifier, and substrate. The outputs you write become the seed data when chat-arch's automation lands — write them so they're machine-readable later.
...
This workflow is what chat-arch v2 Rev 3 (the chat-arch repo's `_planning/chat-arch-v2-rev3-plan.md`) will eventually automate. Until that ships:
...
- **At retro end** for each finding:
  ...
  ```
  `--confidence` is optional and is best omitted, so it is computed from the
