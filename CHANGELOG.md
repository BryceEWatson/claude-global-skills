...
- **`weekly-work-log`: an old open weekly PR is now a failure, and the weekly PR can merge
  ...
  `weekly-pr-guard.cjs` now fails a weekly PR aged seven or more Pacific calendar days as
  `STALE_WEEKLY_PR`, with the PR's link in `last-run.json`, and the Monday preview raises
  ...
  its reason), collects last week's one-word answers, runs the findings miner that used to run
